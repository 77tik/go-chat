...
## 项目BUG：
+ connect层 ws 初始化进etcd中巨慢，而且有的时候ws是不可用的
+ 不仅仅是connect层，只要是注册rpc方法的，都很慢，而且必须得等他们注册好才行，否则整体服务会失效
+ 连接websocket（加入聊天室）会初始化房间号，默认为1，客户端定义默认房间号，也许我们要在服务端做一下扩容房间中间件
+ key 是 tcp@0.0.0.0:6914，有可能服务先以 0.0.0.0 注册（监听所有地址），随后才上报“真实可用地址”。如果发现者在中间时刻查表，可能认为没有可用 IP（或者你的查找逻辑过滤掉 0.0.0.0）。
+ md,啥也不是，只是我connect层过滤serverType的时候把 首字母不小心大写了